...
package com.thelastcheck.ach.generator;
...
import com.thelastcheck.ach.xml.ClassDefinition;
import com.thelastcheck.ach.xml.ClassField;
import com.thelastcheck.ach.xml.X937GenRules;
import com.thelastcheck.ach.xml.X937GenRulesItem;
import com.thelastcheck.ach.xml.ClassDefinitionItem;
import com.thelastcheck.ach.xml.ClassDefinitionList;
import com.thelastcheck.ach.xml.ClassDefinitionListItem;
...
public class GenerateAchClassesXML {
	private static Logger log = LoggerFactory.getLogger(GenerateAchClassesXML.class);
	...
	ClassDefinitionList classDefinitionList = null;
	...
	public static void main(String[] args) {
		new GenerateAchClassesXML().run(args);
	}
	...
	private void run(String[] args) {
		...
		if (rulesFiles == null) {
			rulesFiles = "./src/main/resources/genachrules.txt";
		}
		if (sourceLocation == null) {
			sourceLocation = "./src/main/resources/generated";
		}
		// String packagePath = packageNameImpl.replace('.', '/');
		packageFileImpl = new File(sourceLocation, "");
		packageFileImpl.mkdirs();

		File xmlFile = new File(packageFileImpl, "genrules_ach_impl.xml");
		if (xmlFile.exists()) {
			xmlFile.delete();
		}
		...
		genItem.setClassDefinitionList(classDefinitionList);
		classDefinitionList.setPackage(packageNameImpl+".impl");
		classDefinitionList.setStandard("01");
		...
		for (Iterator<String[]> iter = items.iterator(); iter.hasNext();) {
			...
			if (element[0].equals("C")) {
				newImplClass(element);
			}
			if (element[0].equals("F")) {
				newClassField(element);
			}
		}
	...
	private void newClassField(String[] fieldArgs) {
		...
		field.setNumber(fieldNumberInt);
		field.setLength(fieldLengthInt);
		field.setOffset(fieldStartInt);
		ClassDefinitionItem classDefinitionItem = new ClassDefinitionItem();
		classDefinitionItem.setClassField(field);
		classDefinition.addClassDefinitionItem(classDefinitionItem);
		fieldArrayCount++;
	}
	...
	private void newImplClass(String[] args) {
		...
		classDefinition.setRecordType(Integer.parseInt(recordNumber));

		ClassDefinitionListItem listItem = new ClassDefinitionListItem();
		listItem.setClassDefinition(classDefinition);
		classDefinitionList.addClassDefinitionListItem(listItem);
	}
