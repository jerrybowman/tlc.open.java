...
public class GenerateAchInterfacesXML {
	private static Logger log = LoggerFactory.getLogger(GenerateAchInterfacesXML.class);
	...
	public static void main(String[] args) {
		new GenerateAchInterfacesXML().run(args);
	}
	...
	private void run(String[] args) {
		...
		File xmlFile = new File(packageFileImpl, "genrules_ach.xml");
		if (xmlFile.exists()) {
			xmlFile.delete();
		}
		...
		for (Iterator<String[]> iter = items.iterator(); iter.hasNext();) {
			...
			if (element[0].equals("C")) {
				newInterfaceClass(element);
			}
			if (element[0].equals("F")) {
				newInterfaceField(element);
			}
		}
	...
	private void newInterfaceField(String[] fieldArgs) {
	...
	private void newInterfaceClass(String[] args) {
